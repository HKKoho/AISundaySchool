...
import { generateChallenge } from './challengeService';
import { Timer, Heart, Users, Info, X, User, Footprints, Mountain, Sprout, BookOpen, HandHeart, MessageCircle, AlertCircle, CheckCircle } from 'lucide-react';

interface ParableKeeperGameProps {
  onBack: () => void;
}
...
export const ParableKeeperGame: React.FC<ParableKeeperGameProps> = ({ onBack }) => {
  ...
  const spawnVisitor = useCallback(() => {
    setGameState(prev => {
      ...
      const newVisitor: Visitor = {
        ...
        targetY: Math.random() * (CANVAS_HEIGHT - 250) + 150,
        spawnTime: Date.now(),
        needs: 'GREET',
        testTimer: 0
      };
  ...
  const updateVisitors = useCallback(() => {
    setGameState(prev => {
      ...
      }).filter(v => v.state !== VisitorState.LEAVING_SAD || v.patience > -50);
      ...
      <div className="min-h-screen bg-stone-900 flex flex-col items-center justify-center text-stone-100 p-8">
        <div className="bg-stone-800 p-8 rounded-lg border-2 border-amber-600 max-w-2xl w-full text-center relative">
          ...
          <div className="mb-8">
            ...
            <div className="p-6 bg-stone-900 italic font-serif border-l-4 border-amber-500">
              <p>"{t('gameOver.reflection')}"</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 justify-center">
            ...
            <button
              onClick={onBack}
              className="bg-stone-600 hover:bg-stone-700 text-white px-8 py-3 rounded font-bold uppercase tracking-widest transition-colors"
            >
              {t('buttons.back')}
            </button>
          </div>
  ...
  if (!gameState.isPlaying || !gameState.selectedRole) {
    return (
      <div className="fixed inset-0 bg-stone-900">
        <button
          onClick={onBack}
          className="absolute top-4 left-4 z-50 text-stone-300 hover:text-white transition-colors bg-stone-800 hover:bg-stone-700 px-4 py-2 rounded-lg"
        >
          ← {t('buttons.back')}
        </button>
        <RoleSelect onSelect={handleStartGame} />
      </div>
    );
  }
      ...
      <div className="w-full max-w-[800px] flex justify-between items-center mb-4 bg-stone-800 p-4 rounded-lg border border-stone-700 text-stone-100 shadow-lg relative">
        <div className="flex items-center gap-4">
          <div className="bg-stone-700 px-3 py-1 rounded text-amber-400 font-bold">
            {t(`roles.${gameState.selectedRole.toLowerCase()}.name`)}
          </div>
